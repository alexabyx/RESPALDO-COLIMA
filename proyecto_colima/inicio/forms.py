...
class RegistrarDetallePagoForm(forms.Form):
	...
	pago 				= forms.ModelChoiceField(required=True, label="Pago", help_text="" ,queryset=Pagos.objects.all(), widget=forms.Select(attrs={'type': "select", 'class': "form-control"}))
	detalle_pago 		= forms.ModelChoiceField(required=True, label="Detalle de pago", help_text="" ,queryset=DetallePagos.objects.all().values_list('id', flat=True), widget=forms.Select(attrs={'type': "select", 'class': "form-control"}))
